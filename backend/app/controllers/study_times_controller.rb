class StudyTimesController < ApplicationController
  def show
    ...
    # ②以下、そのユーザーIDを含む学習時間を取得する
    # 今までの合計勉強時間を取得する
    total_study_times = StudyTime.where(user_id: user.id)
    total_study_time = calculate_study_time(total_study_times)

    # 今日の合計学習時間を取得する
    today_study_times = StudyTime.where(user_id: user.id, start_time: Time.zone.now.beginning_of_day..Time.zone.now.end_of_day)
    today_study_time = calculate_study_time(today_study_times)

    # 今週の合計学習時間を取得する
    this_week_study_times = StudyTime.where(user_id: user.id, start_time: Time.zone.now.beginning_of_week..Time.zone.now.end_of_week)
    this_week_study_time = calculate_study_time(this_week_study_times)

    # 先週の合計学習時間を取得する
    last_week_study_times = StudyTime.where(user_id: user.id, start_time: Time.zone.now.prev_week.beginning_of_week..Time.zone.now.prev_week.end_of_week)
  ...
  def show_by_date
    ...
    user = User.find_by(token: params[:token])
    # ②以下、そのユーザーIDを含む学習時間を取得する
    study_times = StudyTime.where(user_id: user.id, start_time: params[:date].to_date.beginning_of_day..params[:date].to_date.end_of_day)
